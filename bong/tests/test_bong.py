...
def toydata(N=60, std1=1, std2=1.1, key=0):
    """Creates a separable toy linear classification dataset"""
...
def test_linearized_bong(key=42, num_timesteps=15):
    ...
    def apply_fn(w, x):
        return jax.nn.sigmoid(linear_model.apply(unflatten_fn(w), jnp.atleast_1d(x)))

    init_cov = 0.1 * jnp.eye(len(flat_params))
    dynamics_cov = 0.0 * jnp.eye(len(flat_params))
    ...
    assert jnp.allclose(ekf_post.filtered_covariances[-1], bong_post.cov, atol=1e-2)
