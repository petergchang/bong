...
def plot_results_from_images_old(
...
def plot_timeseries(
    ...
    for jobname in jobnames:
        ...
        ndx = steps[
            first_step:T:step_size
        ]  #  skip first K time steps, since it messes up the vertical scale
        xs = steps[ndx]
        ys = vals[ndx]
        ...
        if not jitter:
            ax.plot(
                xs, ys, markevery=20, label=expt_name, color=colors[i], **plot_params
            )
            marker = plot_params["marker"]
        else:
            (line,) = ax.plot(
                xs, ys, color=colors[i], label=expt_name
            )  # **plot_params)
            T = len(xs)
            Tstep = int(T / 20)
        ...
        i = (
            i + 1
        )  # This counts actual number of lines, which may be less than njobs due to exclusioh
    ...
    legend_handles = [
        Line2D(
            [0], [0], color=colors[i], marker=markers[i], linestyle="-", label=labels[i]
        )
...
def main(args):
    ...
    for metric in metrics:
        results = extract_results_from_files(
            results_dir, metric, args.jobs_file, args.jobs_suffix
        )
