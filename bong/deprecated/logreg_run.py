...
from bong.src import experiment_utils
from bong.util import MLP, tune_init_hyperparam
...
def generate_logreg_dataset_from_gmm(
    key, N, d, c=1.0, scale=1.0, coef_s=0.2, mean_dir=None
):
...
def generate_logreg_dataset(key, N, d, c=1.0, scale=1.0, theta=None):
    if isinstance(key, int):
        key = jr.PRNGKey(key)
    keys = jr.split(key, 4)
    mean = jnp.zeros(d)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--learning_rate", type=float, nargs="+", default=[0.005, 0.01, 0.05]
    )
    parser.add_argument("--tune_learning_rate", type=bool, default=False)
    parser.add_argument("--num_iter", type=int, nargs="+", default=[10])
