...
from bong.src import bog, experiment_utils
from bong.util import tune_init_hyperparam, gaussian_kl_div
        ...
        if (agent in LR_AGENT_TYPES) and args.tune_learning_rate:
            for n_sample in args.num_samples:
                for n_iter in args.num_iter:
                    ...
                    agent_queue[
                        f"{agent}-M{n_sample}-I{n_iter}-LR{best_lr_str}-tuned"
                    ] = curr_agent
...
def main(args):
    ...
    agent_queue, subkey = make_agent_queue(
        subkey, args, init_kwargs, tune_kl_loss_fn, data["X_tr"], data["Y_tr"]
    )
