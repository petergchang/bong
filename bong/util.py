...
def make_plot_params(algo, ef, lin):
    ...
    if (ef == 0) and (lin == 0):  # sampled Hessians
        ...
        linestyle = "-"
    return {"linestyle": linestyle, "linewidth": 2, "marker": marker, "markersize": 10}
...
def make_bias_initializer(method, minval=-jnp.sqrt(6), maxval=0):
    ...
    def custom_uniform(key, shape, dtype=jnp.float32):
        return jnp.array(
            jax.random.uniform(key, shape, dtype=dtype, minval=minval, maxval=maxval)
        )
...
class MLP(nn.Module):
    ...
    def __call__(self, x):
        ...
        if len(self.features) == 1:  # linear model
            x = nn.Dense(
                self.features[-1],
                use_bias=self.use_bias_first_layer,
                bias_init=self.bias_init_fn_first_layer,
            )(x)
        else:
            x = self.activation(
                nn.Dense(
            ...
            for feat in self.features[1:-1]:
                x = self.activation(
                    nn.Dense(feat, use_bias=self.use_bias, bias_init=self.bias_init_fn)(
                        x
                    )
                )
            x = nn.Dense(
                self.features[-1], use_bias=self.use_bias, bias_init=self.bias_init_fn
            )(x)
        return x
...
def fast_svd(
    ...
    U = M @ (U * vector_pseudo_inverse(jnp.sqrt(S)))
    S = jnp.sqrt(S)
    return U, S
...
def sample_dlr(key, W, diag, temperature=1.0, shape=None):
    ...
    samples = jax.vmap(sample_dlr_single, in_axes=(0, None, None, None))(
