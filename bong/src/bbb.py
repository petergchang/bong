...
class BBBDLRState(NamedTuple):
    """Belief state of a DLR-BBB agent.
...
def init_bbb_dlr(
    ...
    Returns:
        Initial belief state.
    """
    return BBBDLRState(init_mean, init_prec_diag, init_prec_lr)
...
def predict_bbb_dlr(
    ...
    **kwargs,
    ...
    new_state = BBBDLRState(new_mean, new_prec_diag, new_prec_lr)
    return new_state
...
def update_dlrg_bbb(
    ...
    **kwargs,
    ...
    grads = jax.vmap(jax.grad(ll_fn))(z)
    g = jnp.mean(grads, axis=0)
    mean_update = prec_diag0.ravel() * (mean0 - mean) + \
    ...
    lr_term1 = (diag_term11 * prec_lr) @ C \
        - (prec_lr/prec_diag) @ (C @ (prec_lr.T @ (diag_term11 * prec_lr) @ C))
    lr_term2 = -lr_term0 + \
        lr_term0 @ ((prec_lr/prec_diag).T @ prec_lr @ C)
    lr_term3 = (grad_term/prec_diag) @ (grad_term.T @ ((prec_lr/prec_diag) @ C)) \
        - lr_term0 @ (((prec_lr/prec_diag).T @ grad_term) @ grad_term.T
                      @ (prec_lr/prec_diag) @ C)
    lr_term4 = (prec_lr0/prec_diag) @ (prec_lr0.T @ ((prec_lr/prec_diag) @ C)) \
        - lr_term0 @ ((prec_lr/prec_diag).T @ 
                      (prec_lr0 @ (prec_lr0.T @ ((prec_lr/prec_diag) @ C))))
    lr_term5 = (prec_lr/prec_diag) @ (prec_lr.T @ ((prec_lr/prec_diag) @ C)) \
        - lr_term0 @ ((prec_lr/prec_diag).T @ 
...
class dlrg_bbb:
    ...
    def __new__(
        ...
        **kwargs
        ...
        def init_fn() -> BBBDLRState:
            return staticmethod(init_bbb_dlr)(
                init_mean, init_prec_diag, init_lr
            )
            
        def pred_fn(state: BBBDLRState) -> BBBDLRState:
            return staticmethod(predict_bbb_dlr)(
                state, dynamics_decay, process_noise
            )
