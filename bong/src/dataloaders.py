...
def _process(
    ...
    idx = jr.permutation(key, jnp.arange(len(X))) if shuffle else jnp.arange(len(X))
    X, Y = X[idx], Y[idx]
    if n is not None:
        X, Y = X[:n], Y[:n]
    new_args = []
...
def process_dataset(
    ...
    train, val, test = (
        ...
        for dataset, n, key, oh in zip(
            [train, val, test], [ntrain, nval, ntest], keys, [oh_train, False, False]
        )
    )
    dataset = {
        "train": train,
...
def load_base_dataset(
    ...
    **process_kwargs,
    ...
    for ds in [train_ds, val_ds, test_ds]:
        ds["image"] = np.float32(ds["image"]) / 255.0
    ...
    dataset = process_dataset(train, val, test, output_dim=output_dim, **process_kwargs)
    return dataset
