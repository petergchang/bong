...
def logreg_kl_div(key, mu0, cov0, mu, cov, X, Y, n_samples=100):
    ...
    result = -jnp.mean(
        jax.vmap(logreg_posterior, (0, None, None, None, None))(
...
def main(args):
    ...
    def callback(
        ...
        kl_div = logreg_kl_div(
            key, mu0, cov0, state.mean, state.cov, X_tr, Y_tr, n_samples
        )
        # Plugin-NLL
    ...
    nlpd_lap = jnp.mean(jax.vmap(nll_mean_fn, (0, None, None))(means_lap, X_te, Y_te))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_samples", type=int, nargs="+", default=[1, 10, 100])
