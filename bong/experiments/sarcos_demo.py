...
for ntrain in [0]:
    ...
    print(
        f"Estimated noise variance. Gauss {v_y}, Linreg {sigma2}, Linreg-sd {sigma2_sgd}"
    )
    ...
    nll_te_gauss = jnp.mean(
        jax.vmap(nll_gauss, (None, None, 0, 0))(mu_y, v_y, data["X_te"], data["Y_te"])
    )
    nll_te_linreg = jnp.mean(
        jax.vmap(nll_linreg, (None, None, 0, 0))(w, sigma2, data["X_te"], data["Y_te"])
    )
