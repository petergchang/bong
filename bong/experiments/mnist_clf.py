...
def _farray(x):
    x = jnp.array(x)
    x = jnp.where(x < -1e1, jnp.nan, jnp.where(x > 1e1, jnp.nan, x))
    return x
...
def callback_fn(key, alg, state, x, y, em_function, X_cb, y_cb, num_samples=10):
    ...
    nll_pi = jnp.mean(
        optax.softmax_cross_entropy_with_integer_labels(ypi_pred_logits, y_cb)
    )
    ...
    y_pred_logits = jnp.mean(
        jax.vmap(jax.vmap(em_function, (None, 0)), (0, None))(states, X_cb), axis=0
    )
    nll = jnp.mean(optax.softmax_cross_entropy_with_integer_labels(y_pred_logits, y_cb))
    ...
    ylin_pred_logits = jnp.mean(
        jax.vmap(jax.vmap(_linearized_apply, (None, 0)), (0, None))(states, X_cb),
        axis=0,
    )
    nll_lin = jnp.mean(
        optax.softmax_cross_entropy_with_integer_labels(ylin_pred_logits, y_cb)
    )
...
def tune_agents(
    ...
    for agent, (agent_name, kwargs) in agent_queue.items():
        ...
        curr_kwargs = {**kwargs}
        if "-l-" in agent:
            curr_kwargs["emission_mean_function"] = curr_kwargs["em_linpi_function"]
...
def evaluate_agents(key, agent_queue, X_tr, Y_tr, eval_cb_fn, eval_niter, result_path):
    key, subkey = jr.split(key)
    result_dict = {}
    for agent_name, agent in agent_queue.items():
        ...
        for _ in tqdm.trange(eval_niter):
            ...
            _, (nll_pi, miscl_pi, nll, miscl, nll_lin, miscl_lin) = (
                jax.block_until_ready(
                    run_rebayes_algorithm(key2, agent, X_tr, Y_tr, transform=eval_cb_fn)
                )
            )
            # Run without callback to measure runtime
            t0 = time.perf_counter()
            _ = jax.block_until_ready(run_rebayes_algorithm(key2, agent, X_tr, Y_tr))
        ...
        print(
            f"\tPlugin-NLL: {jnp.nanmean(nll_pis):.4f},"
            f" Plugin-Miscl: {jnp.nanmean(miscl_pis):.4f}"
        )
        print(
            f"\tNLPD-NLL: {jnp.nanmean(nlls):.4f},"
            f" NLPD-Miscl: {jnp.nanmean(miscls):.4f}"
        )
        print(
            f"\tLinearized-NLL: {jnp.nanmean(nll_lins):.4f},"
...
def save_results(result_dict, result_path):
    ...
    fig.savefig(Path(result_path, "runtime.pdf"), bbox_inches="tight", dpi=300)
    ...
    ax.grid()
    ax.legend()
    fig.savefig(Path(result_path, "plugin_nlpd.pdf"), bbox_inches="tight", dpi=300)
    ...
    ax.grid()
    ax.legend()
    fig.savefig(Path(result_path, "plugin_miscl.pdf"), bbox_inches="tight", dpi=300)
    ...
    ax.grid()
    ax.legend()
    fig.savefig(Path(result_path, "mc_nlpd.pdf"), bbox_inches="tight", dpi=300)
    ...
    ax.grid()
    ax.legend()
    fig.savefig(Path(result_path, "mc_miscl.pdf"), bbox_inches="tight", dpi=300)
    ...
    ax.grid()
    ax.legend()
    fig.savefig(Path(result_path, "linmc_nlpd.pdf"), bbox_inches="tight", dpi=300)
    ...
    ax.grid()
    ax.legend()
    fig.savefig(Path(result_path, "linmc_miscl.pdf"), bbox_inches="tight", dpi=300)
...
def main(args):
    # Generate dataset
    X_tr, Y_tr, X_val, Y_val, X_te, Y_te = load_mnist_dataset(args.n_train, args.n_test)

    # Set up MLP model
    model_init_kwargs, key = initialize_model(
        args.seed, args.features, X_tr[0], args.model
    )
    ...
    tune_loss_fn = partial(loss_fn, em_function=em_function, X_val=X_val, y_val=Y_val)
    ...
    eval_cb_fn = partial(callback_fn, em_function=em_function, X_cb=X_te, y_cb=Y_te)
    result_dict = evaluate_agents(
        key, agent_queue, X_tr, Y_tr, eval_cb_fn, args.eval_niter, result_path
    )
...
if __name__ == "__main__":
    ...
    # Model parameters
    parser.add_argument(
        "--agents", type=str, nargs="+", default=["dg-bong"], choices=BONG_DICT.keys()
    )
    parser.add_argument(
        "--dlr_ranks",
        type=int,
    ...
    parser.add_argument("--num_samples", type=int, nargs="+", default=[100])
    parser.add_argument("--num_iters", type=int, nargs="+", default=[10])
