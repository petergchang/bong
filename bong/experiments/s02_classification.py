...
from bong.util import MLP, run_rebayes_algorithm
...
def generate_logreg_dataset(key, N, d, c=1.0, scale=1.0, coef_s=0.2, mean_dir=None):
...
def generate_uci_dataset(key, dataset_name, n_train, n_test):
    if isinstance(key, int):
        key = jr.PRNGKey(key)
...
def logreg_posterior(
    mu, mu0, cov0, X, Y, em_function, nll_fn=optax.sigmoid_binary_cross_entropy
):
    def _ll(x, y):
...
def logreg_kl_div(
    ...
    result = -jnp.mean(
        jax.vmap(logreg_posterior, (0, None, None, None, None, None))(
...
def main(args):
    ...
    if args.dataset == "logreg":
        ...
        X_val, Y_val, _ = generate_logreg_dataset(
            key2, args.n_test, d, mean_dir=mean_dir
        )
        X_te, Y_te, _ = generate_logreg_dataset(key3, args.n_test, d, mean_dir=mean_dir)
        # Set up Logistic Regression model
        eps = 1e-5
        sigmoid_fn = lambda w, x: jnp.clip(jax.nn.sigmoid(w @ x), eps, 1 - eps)
        log_likelihood = lambda mean, cov, y: -optax.sigmoid_binary_cross_entropy(
            mean, y
        )
        ...
        def tune_kl_loss_fn(key, alg, state):
            return logreg_kl_div(
                key, mu0, cov0, state.mean, state.cov, X_val, Y_val, 100, nll_fn
            )
    else:
    ...
    def callback(
        key, alg, state, x, y, mu0=mu0, cov0=cov0, X_cb=X_te, Y_cb=Y_te, n_samples=100
    ):
        # KL-div
        kl_div = logreg_kl_div(
            key,
            mu0,
            cov0,
            state.mean,
    ...
    if args.dataset == "logreg":
        ...
        key, subkey = jr.split(subkey)
        sol_lap = minimize(
            lambda *args: -logreg_posterior(*args),
            mu0,
            args=(mu0, cov0, X_tr, Y_tr, em_function),
            method="BFGS",
        ...
        mu_lap = sol_lap.x
        nll_sum_fn = lambda m, X, Y: jnp.sum(jax.vmap(_nll, (None, 0, 0))(m, X, Y))
        nll_mean_fn = lambda m, X, Y: jnp.mean(jax.vmap(_nll, (None, 0, 0))(m, X, Y))
        cov_lap = jnp.linalg.pinv(jax.hessian(nll_sum_fn)(mu_lap, X_tr, Y_tr))
        kldiv_lap = logreg_kl_div(
            key, mu0, cov0, mu_lap, cov_lap, X_tr, Y_tr, em_function
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="logreg",
        choices=["logreg", "uci-statlog-shuttle", "uci-covertype", "uci-adult"],
    )
