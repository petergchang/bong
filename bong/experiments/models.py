...
from bong.util import gaussian_kl_div, MLP
from job_utils import parse_neuron_str
...
gauss_log_likelihood = lambda mean, cov, y: jax.scipy.stats.norm.logpdf(
...
def fit_linreg_baseline(Xtrain, ytrain, method="lstsq"):
    ...
    if method == "lstsq":
        ...
        )  # We assume column of 1s has been added
...
def compute_reg_baselines(args, data, obs_var):
    ...
    nll_te_linreg_obs_var = jnp.mean(
        jax.vmap(nll_linreg, (None, None, 0, 0))(w, obs_var, Xte, ytest)
    )
    nll_te_linreg_mle_var = jnp.mean(
        jax.vmap(nll_linreg, (None, None, 0, 0))(w, sigma2, Xte, ytest)
    )
    results["nll_te_linreg"] = (
        nll_te_linreg_obs_var  # to be comparable with MLP methods
    )
    ypred = Xte @ w
    ...
    print(
        f"mse={mse:0.3f}, smse={smse:0.3f}, nll_obsvar={nll_te_linreg_obs_var:0.3f}, nll_mlevar={nll_te_linreg_mle_var:0.3f}"
    )
...
def callback_reg(
    ...
    nlpd_te = jnp.mean(
        jax.vmap(jax.vmap(_nll, (None, 0, 0)), (0, None, None))(means, X_te, Y_te)
    )

    key, subkey = jr.split(key)
    means = alg.sample(subkey, state, n_samples_mc_nlpd)
    nlpd_val = jnp.mean(
        jax.vmap(jax.vmap(_nll, (None, 0, 0)), (0, None, None))(means, X_val, Y_val)
    )
...
def make_mlp_reg(key, args, data):
    ...
    key, subkey = jr.split(key)
    key, model_kwargs = initialize_mlp_model_reg(
        subkey,
        neurons,
        data["X_tr"][0],
        args.init_var,
...
def initialize_mlp_model_reg(
    key, features, x, init_var, emission_noise, use_bias=True, use_bias_layer1=True
):
    if isinstance(key, int):
        key = jr.key(key)
    key, subkey = jr.split(key)
    model = MLP(
        features=features, use_bias=use_bias, use_bias_first_layer=use_bias_layer1
    )
...
def make_mlp_cls(key, args, data):
    neurons = parse_neuron_str(args.model_str)
    key, subkey = jr.split(key)
    key, model_kwargs = initialize_mlp_model_cls(
        subkey, neurons, data["X_tr"][0], args.init_var, args.use_bias
    )
    nparams = model_kwargs["nparams"]
    model_name = f"mlp_{args.model_str}[P={nparams}]"
    ...
    def process_callback(output):
        nll_te, nll_val, acc_pi_te, acc_pi_val, nlpd_te, nlpd_val, acc_te, acc_val = (
            output
        )
        ...
        results = {
            ...
            "acc-pi": acc_pi_te,
            "acc-mc": acc_te,
            "nllpd-pi_val": nll_val,
            "nlpd-mc_val": nlpd_val,
            "acc-pi_val": acc_pi_val,
            "acc-mc_val": acc_val,
        }
        return results, summary
...
def callback_cls(
    key, alg, state, x, y, em_function, X_te, y_te, X_val, y_val, num_samples=100
):
    # Plugin-LL
    ypi_pred_logits_te = jax.vmap(em_function, (None, 0))(state.mean, X_te)
    ll_pi_te = jnp.mean(
        -optax.softmax_cross_entropy_with_integer_labels(ypi_pred_logits_te, y_te)
    )

    ypi_pred_logits_val = jax.vmap(em_function, (None, 0))(state.mean, X_val)
    ll_pi_val = jnp.mean(
        -optax.softmax_cross_entropy_with_integer_labels(ypi_pred_logits_val, y_val)
    )
    ...
    y_pred_logits_te = jnp.mean(
        jax.vmap(jax.vmap(em_function, (None, 0)), (0, None))(states, X_te), axis=0
    )
    ll_te = jnp.mean(
        -optax.softmax_cross_entropy_with_integer_labels(y_pred_logits_te, y_te)
    )

    key, subkey = jr.split(key)
    states = alg.sample(subkey, state, num_samples)
    y_pred_logits_val = jnp.mean(
        jax.vmap(jax.vmap(em_function, (None, 0)), (0, None))(states, X_val), axis=0
    )
    ll_val = jnp.mean(
        -optax.softmax_cross_entropy_with_integer_labels(y_pred_logits_val, y_val)
    )
