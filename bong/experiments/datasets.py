...
def get_sarcos_data(key=0, ntrain=0, nval=0, ntest=0, raw=False, add_ones=False):
    ...
    if raw:
        data = {
            "X_train_raw": data_train[:, :21],
            "Y_train_raw": data_train[:, 21],
            "X_test_raw": data_test[:, :21],
            "Y_test_raw": data_test[:, 21],
        }
        return data
...
def generate_xdata_mvn(key, N, d, c=1.0, scale=1):
...
def generate_linear_model(key, d):
    key1, key2, key = jr.split(key, 3)
    theta = jr.uniform(key1, (d,), minval=-1.0, maxval=1.0)
...
def make_data_reg_mlp(key, args):
    ...
    key, model = initialize_mlp_model_reg(
        key0, neurons, x, args.init_var, args.emission_noise
    )
