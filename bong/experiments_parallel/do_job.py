...
def run_agent(key, agent, data, model):
    ...
    D = jnp.prod(sh[1:])
    print(
        f"Running {agent.name} + {model['name']} on {data['name']} of size N={N}, D={D}"
    )
    print("Using GPU of type: ", get_gpu_name())
    t0 = time.perf_counter()
    _, output = jax.block_until_ready(
        run_rebayes_algorithm(
            key, agent, data["X_tr"], data["Y_tr"], transform=model["callback"]
        )
    )
...
def main(args, args_dict):
    ...
    fname = Path(results_path, "dummy.txt")
    ...
    fname = Path(results_path, "args.json")
    args_dict = args_dict | meta
    with open(fname, "w") as json_file:
        json.dump(args_dict, json_file, indent=4)

    fname = Path(results_path, "results.csv")
...
if __name__ == "__main__":
    ...
    args_dict = {
        action.dest: getattr(args, action.dest, None) for action in parser._actions
    }
    args_dict.pop("help")
    main(args, args_dict)
